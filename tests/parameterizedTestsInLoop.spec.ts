...
deTest.describe("Parameterized Tests in Loop @S49731617", () => {
    ...
    for (const testValue of testValues1) {
        ...
        deTest(`API 1.0 - TestA - ${fruitName} @Tf14fa20d`, async ({ deLog, deWaiter }) => {
            deLog.logStart();
            await deWaiter.waitSeconds(1);
            deExpect(1 + 1).toBe(2);
            deLog.logFinish();
        });
        deTest(`API 1.0 - TestB - ${fruitName} @T14e9c507`, async ({ deLog, deWaiter }) => {
            deLog.logStart();
            await deWaiter.waitSeconds(1);
            deExpect(1 + 1).toBe(2);
            deLog.logFinish();
    ...
    for (const testValue of testValues2) {
        deTest(`API 2.0 - TestA - ${testValue} @Tfc05ed0d`, async ({ deLog }) => {
            deLog.logStart();
            deExpect(1 + 1).toBe(2);
            deLog.logFinish();
        });
        deTest(`API 2.0 - TestB - ${testValue} @T54cb7790`, async ({ deLog }) => {
            deLog.logStart();
            deExpect(1 + 1).toBe(2);
            deLog.logFinish();
        });
